...
export class PackagesService {
    ...
    async getPackageMenus(packageId: string) {
        ...
        // Get all menu IDs including inherited ones
        const menuIds = await this.getInheritedMenuIds(packageId);

        // Get full menu items using In operator
        const menus = await this.menuItemsRepository.find({
            where: {
                id: In(menuIds),
                is_active: true,
                is_available: true,
    ...
    private async getInheritedMenuIds(packageId: string): Promise<string[]> {
        const pkg = await this.packagesRepository.findOne({
            where: { id: packageId },
            relations: ['parent_package'],
        });
