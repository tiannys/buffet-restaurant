...
export class BillingService {
    ...
    async calculateBill(sessionId: string) {
        ...
        const vatPercent = Number(settings.vat_percent);
        const serviceChargePercent = Number(settings.service_charge_percent);
        ...
        // Get PromptPay QR URL if configured
        const promptpayQrUrl = await this.settingsService.getPromptPayQrUrl(grandTotal);
    ...
    async createReceipt(data: {
        ...
        if (data.member_id && data.points_used && data.points_used > 0) {
            const pointsPerBaht = 1; // TODO: Move to settings
        ...
        if (data.member_id) {
            const pointsPerBaht = 0.01; // TODO: Move to settings
