...
export class SessionsController {
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles('Cashier', 'Admin')
    endSession(@Param('id') id: string, @Body() paymentData: any, @Request() req) {
        return this.sessionsService.endSession(id, req.user.userId, paymentData);
    }
    ...
    getSessionsNeedingWarning() {
        return this.sessionsService.getSessionsNeedingWarning();
    }

    @Get(':id/warnings')
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles('Staff', 'Admin', 'Cashier')
    checkTimeWarnings(@Param('id') id: string) {
        return this.sessionsService.checkTimeWarnings(id);
    }

    @Post(':id/mark-warning-sent')
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles('Staff', 'Admin')
    markWarningAsSent(@Param('id') id: string) {
        return this.sessionsService.markWarningAsSent(id);
    }
